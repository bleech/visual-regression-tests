					...
					<ul style="list-style-type: disc; margin: 0; padding-left: 1.5em">
						<?php foreach ( $tests_without_alerts as $test_id ) : ?>
							...
							<li style="margin-top: 10px">
								<span style="font-size: 14px; font-weight: 600; text-align: left; line-height: 20px; mso-line-height-rule: exactly"><?php esc_html( get_the_title( $test->post_id ) ); ?></span><br />
								<a href="<?php echo esc_url( get_the_permalink( $test->post_id ) ); ?>" style="font-size: 14px; font-weight: 400; color: #1954ed; text-align: left; line-height: 20px; mso-line-height-rule: exactly; text-decoration: none;"><?php echo esc_url( Url_Helpers::get_relative_permalink( $test->post_id ) ); ?></a>
							</li>
						<?php endforeach; ?>
					</ul>
				</td>
